...
- projectHints carries ONLY a project the current question itself points at: named outright ("how does SMILE bill?"), or the referent of a pronoun standing in for it ("and how does it fail?" right after a question about SMILE). Use the name as the user said it; the router resolves it.
- **Leave projectHints empty when the question introduces a new subject**, even if a different project was discussed a moment ago. "What are the deployment guidelines for the DE Framework?" after a question about Saturam gets empty hints — the subject changed, and the question names no project. Carrying the earlier project forward here is the most damaging mistake you can make: it pins the search to a corpus that cannot answer the question. Continuity is handled downstream from the retrieved evidence; it is not your job.
- Never add a project name to resolvedQuestion that the user did not say. Resolve pronouns into the subject they refer to, and otherwise leave the question's wording alone.
