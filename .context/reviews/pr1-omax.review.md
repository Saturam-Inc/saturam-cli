...
**Should fix:** 6. `additions`/`deletions` hardcoded to 0 with no TODO 7. `GITLAB_MR_REGEX` subgroup and host-anchoring issues 8. GitLab MR URL host not carried through to API calls 9. `diff_refs` nullability not handled

**Nice to have:** 10. Token logging reduction 11. URL util relocation 12. Redundant MR fetch elimination 13. `Promise.allSettled` for parallel comment posting
